...
for gg in np.arange(1,3,1):
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            ...
            latency = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['Latency']].to_numpy()
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            # Create a temporary variable
            speed = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['Av_speed']].to_numpy()
            # Get the average for each one of the day-trial combination (16)
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            ...
            # PRIMARY ERROR
            # Create a temporary variable
            p_err = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['P_error']].to_numpy()
            # Get the average for each one of the day-trial combination (16)
            ...
            # SECUNDARY ERROR
            # Create a temporary variable 
            s_err = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['S_error']].to_numpy()
            # Get the average for each one of the day-trial combination (16)
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            # Create a temporary variable
            distance = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['Distance']].to_numpy()
            # Get the average for each one of the day-trial combination (16)
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            # Create a temporary variable
            time_on_target = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['Time_on_target']].to_numpy()
            # Get the average for each one of the day-trial combination (16)
    ...
    for ii in np.arange(1,5,1):
        for jj in np.arange(0,len(IDs),1):
            # Create a temporary variable
            strategy = trial_info.loc[(trial_info["Day"] == str(ii)) & (trial_info["ID_unique"] == IDs[jj]) & (trial_info["Group"] == str(gg)),['Strategy']].to_numpy()
